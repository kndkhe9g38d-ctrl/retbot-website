...
const API_BASE = (CONFIG.API_BASE || window.location.origin).replace(/\/$/,"");
const state = {
...
function applyLoginLocale(){
  ...
  $("#loginNote").textContent=state.locale==="ar"?"تسجيل دخول آمن عبر Discord.":"Secure sign-in with Discord.";
}
function bindLogin(){
  $("#loginBtn").onclick=()=>{
    if(!API_BASE) return toast(t("apiMissingText"),"error");
